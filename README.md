...
**Trigger 1 — update the updater, then push:**

```bash
# after changing the updater or subscription source, then:
git add update-stash2-preserve-groups.js update-stash2-ci.js
git commit -m "Update Huahe SSR nodes"
git push
```

Pushing a change to the updater runs the workflow, which downloads the current
Huahe Clash subscription and the current `stash2.yaml` from the `clashpersonal` repo, merges the latest
